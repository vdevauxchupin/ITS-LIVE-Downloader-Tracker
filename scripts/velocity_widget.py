...
class ITSLIVE:
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.ymin = 0
        self.ymax = 0
        self.points = [(self.ymin, self.xmin), (self.ymax, self.xmax)]       
        self.point_center =  Point(0,0)
        self.xmin_proj = 0
        self.xmax_proj = 0
        self.ymin_proj = 0
        self.ymax_proj = 0  
        # Import shapefile of regions
        self.regions = gpd.GeoDataFrame.from_file('scripts/Regions.shp')
        self.url_region = []
        self.name_region = []
        self.proj_region = []
    ...
    def _round_decimals(self, number:float, sign, decimals:int=2):
    ...
    def _handle_map_click(self, *args, **kwargs):
        ...
        if kwargs.get('type') == 'contextmenu':
            self.map.add_layer(ipyleaflet.Marker(location=kwargs.get('coordinates')))
            self.coord_rectangle.append(kwargs['coordinates'])
            
            
            if len(self.coord_rectangle)%2 == 0: 
                # Count the amount of rectangles
                self.rectangle_amount += 1
                # Create a rectangle from the two points that were selected with the right click       
                rectangle = ipyleaflet.Rectangle(bounds=((self.coord_rectangle[self.counter+1][0], self.coord_rectangle[self.counter+1][1]), (self.coord_rectangle[self.counter][0], self.coord_rectangle[self.counter][1])))
                self.map.add_layer(rectangle)
                # Round up or down the coordinates, so it doesn't crop down on the area people select
                self.xmin = min(self.coord_rectangle[self.counter][1], self.coord_rectangle[self.counter+1][1])
                self.xmax = max(self.coord_rectangle[self.counter][1], self.coord_rectangle[self.counter+1][1])
                self.ymin = min(self.coord_rectangle[self.counter][0], self.coord_rectangle[self.counter+1][0])
                self.ymax = max(self.coord_rectangle[self.counter][0], self.coord_rectangle[self.counter+1][0])
                # Adjust the longitudes, because the map can give longitudes > 180 degrees if you scroll too much to the right
                self.xmin = self._round_decimals((self.xmin - int(self.xmin/180)*360), 0, 2)
                self.xmax = self._round_decimals((self.xmax - int(self.xmax/180)*360), 1, 2)
                self.ymin = self._round_decimals(self.ymin, 0, 2)
                self.ymax = self._round_decimals(self.ymax, 1, 2)
                # Update the point
                self.points = [(self.ymin, self.xmin), (self.ymax, self.xmax)]  
                # Calculate point at the center of the AOI
                self.point_center = Point((self.xmax - self.xmin)/2 + self.xmin, (self.ymax - self.ymin)/2 + self.ymin)
                # Grab the url of the region in which the AOI is
                self.url_region = [self.regions.URL[i] for i in range(len(self.regions)) if self.point_center.within(self.regions.geometry[i])]
                # Grab the name of the region in which the AOI is
                self.name_region = [self.regions.Name[i] for i in range(len(self.regions)) if self.point_center.within(self.regions.geometry[i])]
                # Grab the name of the region in which the AOI is
                self.proj_region = [self.regions.EPSG[i] for i in range(len(self.regions)) if self.point_center.within(self.regions.geometry[i])]
                # Convert boundary coordinates
                # Get the boundaries based on the cubes' projection containing the boundaries (will not work if user is asking for data overlapping 2 projections)
                boundaries = []
                transformer = Transformer.from_crs(4326, int(self.proj_region[0]))
                for pt in transformer.itransform(self.points): boundaries.append(pt)
                boundaries = np.array(boundaries)
                self.xmin_proj = min(boundaries[:,0])
                self.xmax_proj = max(boundaries[:,0])
    ...
    def plot_point_on_fig(self, point_xy, map_epsg):
        ...
        # Gather projections of datacubes containing the points
        self.projections = [xr.open_dataset(self.dct.addresses[i], engine='zarr').projection for i in range(len(self.dct.addresses))]
    ...
    def clear_points(self, *args, **kwargs):
        ...
        self.map.remove_layer(self.map.layers[2+(3*self.rectangle_amount)])
        self.map.remove_layer(self.map.layers[1+(3*self.rectangle_amount)])
        self.map.remove_layer(self.map.layers[3*self.rectangle_amount])
        self.rectangle_amount -= 1
        self.coord_rectangle = self.coord_rectangle[0:len(self.coord_rectangle)-2]
        self.counter -= 2
        
        
        print("all points cleared")
        print(self.coord_rectangle)
